...
use metamui_primitives::{
	traits::{DidResolve, IsMember, IsValidator, MultiAddress},
	types::{PrivateDidVC, PublicDidVC, SlashMintTokens, TokenTransferVC, VCType, VC},
	Did, VCHex, VCid,
};
use sp_core::sr25519;
use sp_runtime::{
...
pub mod pallet {
	...
	pub(super) type VCs<T: Config> =
		StorageMap<_, Blake2_128Concat, VCid, Option<VC<T::Hash>>, ValueQuery>;
	...
	pub(super) type VCHistory<T: Config> =
		StorageMap<_, Blake2_128Concat, VCid, Option<(IsVCActive, T::BlockNumber)>, ValueQuery>;

	/// map for vc id and approvers list
	#[pallet::storage]
	pub(super) type VCApproverList<T: Config> =
		StorageMap<_, Blake2_128Concat, VCid, Vec<Did>, ValueQuery>;
			...
			match vc.vc_type {
				...
				VCType::GenericVC => {
					...
					// ensure the caller is a council member account
						...
						<T as pallet::Config>::IsCouncilMember::is_collective_member(&sender_did),
	...
	fn validate_vcs(vc: &VC<T::Hash>, sender_did: &Did) -> Result<(), DispatchError> {
		match vc.vc_type {
			...
			VCType::SlashTokens | VCType::MintTokens => {
				...
				let token_vc_struct = if let Some(vc_struct) = VCs::<T>::get(&slash_or_mint.vc_id) {
					vc_struct
				} else {
			...
			VCType::TokenTransferVC => {
				...
				let token_vc_struct = if let Some(vc_struct) = VCs::<T>::get(&transfer_tokens.vc_id)
				{
					vc_struct
				} else {
					fail!(Error::<T>::LinkedVCNotFound);
			...
			VCType::PrivateDidVC => {
				...
				let did_region = <T as pallet::Config>::IsValidator::get_region(vc_property.did);
					...
					<T as pallet::Config>::IsValidator::has_regional_permission(
						sender_did, did_region
					),
