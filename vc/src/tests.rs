...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		CouncilCollective: pallet_collective::<Instance1>::{Pallet, Call, Storage, Origin<T>, Event<T>, Config<T>},
		ValidatorCollective: pallet_validator_collective::<Instance2>::{Pallet, Call, Storage, Origin<T>, Event<T>, Config<T>},
	}
);
...
impl Config for Test {
	...
	type IsCouncilMember = CouncilCollective;
	type IsValidator = ValidatorCollective;
...
impl pallet_validator_collective::Config<ValidatorCollective> for Test {
	...
	type Proposal = Call;
	type DidResolution = Did;
	type CallOrigin = MoreThanHalfCouncil;
...
fn new_test_ext() -> sp_io::TestExternalities {
	...
	validator_set::GenesisConfig { validators: vec![ALICE] }
		.assimilate_storage::<Test>(&mut o)
		.unwrap();
	...
	collective::GenesisConfig { members: vec![BOB] }
		.assimilate_storage::<Test>(&mut o)
		.unwrap();
