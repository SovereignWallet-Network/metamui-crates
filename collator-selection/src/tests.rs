...
fn cannot_register_candidate_if_too_many() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			CollatorSelection::register_candidate(Origin::signed(3), 3),
		...
		// but no more
		assert_noop!(
			CollatorSelection::register_candidate(Origin::signed(5), 5),
			Error::<Test>::TooManyCandidates,
		);
	})
}
...
fn cannot_register_as_candidate_if_invulnerable() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			CollatorSelection::register_candidate(Origin::signed(1), 1),
...
fn cannot_register_as_candidate_if_keys_not_registered() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			CollatorSelection::register_candidate(Origin::signed(7), 7),
			Error::<Test>::ValidatorNotRegistered
		);
	})
}
...
fn cannot_register_as_candidate_if_poor() {
	new_test_ext().execute_with(|| {
		...
		// poor
		assert_noop!(
			CollatorSelection::register_candidate(Origin::signed(33), 33),
